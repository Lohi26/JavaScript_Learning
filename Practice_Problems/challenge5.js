...
{
    const correctedJuliaData=juliaData.slice(1,juliaData.length-1);
    const totalData=[...correctedJuliaData,...kateData];
    for(const [i,arr] of totalData.entries())
    {
        if(arr>=3)
        {
        ...
        {
            console.log(`Dog number ${i+1} is still a puppy and is ${arr} years old`);
        }
    }
};
